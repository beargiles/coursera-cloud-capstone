 ...
 * - collect statistics corresponding to AirlineFlightDelaysWritable
 * - select airline_id,count(1),sum(arrival_delay),sum(arrival_delay*arrival_delay),max(arrival_delay) from ontime_info where cancelled=false and diverted=false group by airline_id order by airline_id;
 * 
 * @author bgiles
 */
@Repository
